...
tr3 <- tr_mi[[3]] %>% 
  select(-species)
hist(tr3$LitterSize_n) #continuous
hist(log10(tr3$LitterSize_n)) #still very skeweed as most are  less than 1
hist(sqrt(tr3$LitterSize_n)) #still same - leave be
...
td_mi <- tr_mi[[3]] %>% 
  ...
  dplyr::select(-LitterSize_n, -breeding_site, -vertical, -DielActivity) %>% 
...
#for future scenario remove extinct species from eco
eco <- eco %>% 
  filter(!species %in% no_species)
...
mean(fr_mi$spp/fr_mi$utc) #1.067 for 5 traits
```
...
#utc
mean(Fct_df$spp/Fct_df$utc) #1.004724
```
...
points_tradeoff <- ggplot(Fct_df2, aes(x = fred_ses, y = fdisp_ses, col = tradeoff)) +
  ...
  theme_minimal()
points_tradeoff
ggsave(filename = 'tradeoff_points-5traits-2.png', plot = points_tradeoff, path = '~/R/thesis/FctTradeoffs', 
...
p_tradeoff
ggsave(filename = 'MEOW_tradeoff_map-eco-5traits-2.png', plot = p_tradeoff, path = '~/R/thesis/FctTradeoffs', 
       width = 15, height = 10, unit = 'cm')
...
ggsave(plot = MPA_p1, 'FRed+MPAs3.png', width=15, height=10, units = 'cm')
...
ggsave(plot = MPA_p5, 'Tradeoff+MPAs2.png', width=15, height=10, units = 'cm')
